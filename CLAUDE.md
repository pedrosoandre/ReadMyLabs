...
- Arquivos deployados: `index.php`, `analisar.php`, `.htaccess`, `db.php`, `loads_env.php`, `lib/` (inclui `crypto.php` F4), `auth/` (subsistema completo: `lib/` incluindo `exames_repo.php` F4 + `schema_auth.sql` + `schema_f4.sql` + `README.md`), `email/lib/`, páginas auth (`cadastro.php`, `entrar.php`, `sair.php`, `recuperar.php`, `redefinir.php`, `confirmar.php`, `conta.php`, `auth-status.php`), páginas F4 (`historico.php`, `historico-ver.php`, `historico-apagar.php`, `apagar-conta.php`), `rag/lib/` (runtime do RAG). `rag/ingest/` roda **no servidor** via SSH (o MySQL só aceita `localhost`); é CLI-only (`PHP_SAPI` guard → 403 no web), sobe só para rodar a ingestão. Mesmo padrão CLI-only para `auth/aplicar_schema_f4.php` e `auth/gerar_chave.php` — aplicadores one-off que devem ser **removidos do servidor** após uso.
