...
- **F2-F4 (planejadas, fora do escopo F1):** F2 adiciona Google OAuth + Magic link. F3 substitui o paywall por gateway real (Stripe/MP/Asaas atrás de `email/`-style interface). F4 acrescenta histórico de análises por usuário — **requer revisão LGPD prévia** (hoje exames não são persistidos por design; mudar isso muda o perfil de risco do app).
...
- Arquivos deployados: `index.html`, `analisar.php`, `.htaccess`, `db.php`, `loads_env.php`, `lib/`, `auth/` (subsistema completo: `lib/` + `schema_auth.sql` + `README.md`), `email/lib/`, páginas auth (`cadastro.php`, `entrar.php`, `sair.php`, `recuperar.php`, `redefinir.php`, `confirmar.php`, `conta.php`, `auth-status.php`), `rag/lib/` (runtime do RAG). `rag/ingest/` roda **no servidor** via SSH (o MySQL só aceita `localhost`); é CLI-only (`PHP_SAPI` guard → 403 no web), sobe só para rodar a ingestão.
